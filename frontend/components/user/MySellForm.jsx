...
export default function CartPage() {
    ...
    useEffect(() => {
        if (typeof window !== "undefined") {
            ...
            if (storedUser) {
                ...
                if (!nUser || !nUser.id || nUser.role !== "user") {
                    alert("❌ يجب تسجيل الدخول كمستخدم لعرض السلة");
                    setCartItems([]);
                    setLoading(false);
                    return;
                }
                fetch(`${apiUrl}/cart?userId=${nUser.id}`)
                    .then((res) => res.json())
                    .then((data) => {
                        if (data.cart?.items) {
    ...
    return (
        <div>
            <Header />
            <div className="min-h-screen bg-gray-100 p-6 mt-[-20px]">
                            ...
                            <div key={item._id} className="bg-white rounded-xl shadow-md overflow-hidden">
                                <div className="w-full pt-2 flex justify-center">
                                    <img
                                        ...
                                        className="w-[70%] h-[200px] rounded-2xl"
                                    />
                                </div>
                    ...
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                        <div className="bg-white p-6 rounded shadow-md w-80">
                            ...
                            <button
                                ...
                                className="bg-blue-600 text-white px-4 py-2 rounded w-full mt-1"
                            >
                                تأكيد الشراء
                            </button>
                            <button
